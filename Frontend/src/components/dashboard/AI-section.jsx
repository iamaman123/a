...
import { Orbit, Sparkles, MessageSquare, FileText } from "lucide-react";
...
export default function AISection() {
    ...
    return (<section className="w-full py-8 sm:py-12 flex justify-center">
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: [0.19, 1, 0.22, 1] }} className="
          ...
          max-w-6xl w-full
        ">
        {aiServices.map((service, i) => (<motion.div key={service.name} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }}>
